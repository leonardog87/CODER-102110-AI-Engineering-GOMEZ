...
$namespace = "ciudad-analitica"
$deployment = "ciudad-analitica"
...
$tempRoot = Join-Path ([System.IO.Path]::GetTempPath()) (
    "ciudad-analitica-k8s-" + [guid]::NewGuid().ToString("N")
)
...
try {
    ...
    try {
        kubectl kustomize edit set image "ciudad-analitica=$Image"
    }
    finally {
        Pop-Location
    }
    ...
    kubectl -n $namespace create secret generic ciudad-analitica-secrets `
